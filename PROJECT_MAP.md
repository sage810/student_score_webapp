...
| teacher.html | index.html과 같은 색·글꼴을 쓰지만, 비밀번호 인증 뒤 명단·과목·영역·점수·결시·결시자 수행안내(PDF)·엑셀 다운로드를 관리하는 교사용 화면. 저장하지 않은 변경이 있으면 로그아웃·창 닫기 때 경고함. 로그인 직후 8개 섹션을 바로 펼치는 대신 통계 3개(맡은 과목·결시 대기·이번 수행 완료)와 준비/진행/확인·출력 3단 카드 그리드로 된 홈 화면이 먼저 뜨고, 상단 고정바 "홈" 버튼으로 돌아올 수 있음. "과목" 드롭다운은 항상 아래로 열리는 커스텀 콤보로 통일. 반별 점수 입력표는 결시 칸도 직접 점수 입력 가능(입력 시 결시 자동 해제), 배점 초과 시 저장 차단 + 배지 안내, 30% 이하 저배점은 확인창으로 재확인 |
